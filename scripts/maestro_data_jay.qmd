...
# Load metadata-------------------------------------------------------------------------
sw_meta <- read_csv("C:/Users/User/Desktop/Grad School/lab_work/sup_data/data/sw_metadata.csv") |>
...
```{r}
#import temp file ----------------------------------------------------------------------------
mat_df <- readMat.default(con = "C:/Users/User/Desktop/Grad School/lab_work/sup_data/data/metadata/LSEO24h_temp.mat")
...
test <- temp_df_long |> 
  ...
  mutate(coeff_var_under10 = ifelse(coeff_var > 10, "FALSE", "TRUE")) |> 
  ggplot(aes(dttm, temp, color = coeff_var_under10)) +
  geom_point() +
  facet_wrap(~sample_date, scales = "free") +
  scale_x_datetime(date_labels = "%H")

test
plotly::plotly_build(test)
...
maestro_df <- readMat.default(con = "C:/Users/User/Desktop/Grad School/lab_work/sup_data/data/metadata/lseo_maestro_time.mat") |> 
  data.frame() |> 
...
long_maestro_df <- maestro_df |> 
  ...
  # Quick plot
test <- long_maestro_df |> 
  ggplot(aes(dttm, vals, color = variable)) +
  geom_point()+
  facet_wrap(~variable, scales = "free")
test

# ggsave("output/plots/maestro_raw.jpg", test)
```
...
```{r}
# Function that takes a data frame and variable, checks outliers using a
# absolute deviation around the median (MAD score). Outputs a plot or interactive plotly plot
plot_outliers <- function(maestro_df, var, plotly = F) {
  ...
  outlier_df <- outlier_df |> 
    mutate(outlier = check_outliers(outlier_df, method = "zscore_robust"))
...
maestro_df |> 
  filter(date(dttm) > "2025-06-10" & date(dttm) < "2025-06-17") |> 
  mutate(morning = as.logical(ifelse(hour(dttm) < 12, "TRUE", "FALSE"))) |> 
  ggplot(aes(dttm, PCO2, color = morning)) +
  geom_point()
maestro_df_clean <- maestro_df_clean |> 
