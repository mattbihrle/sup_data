...
mt_16s$sample_table <- mt_16s$sample_table |> 
  mutate(temp_quant = cut_number(temp_c_24h_med, n = 4))
tb2 <- trans_beta$new(mt_16s, group = "temp_quant", measure = "bray")
tb2$cal_ordination(method = "NMDS")
tb2$cal_manova(manova_all = TRUE)
print(tb2$res_manova)
...
plot <- tb2$plot_ordination(plot_color = "temp_quant", plot_shape = "strat_season", NMDS_stress_text_prefix = "Stress: ", plot_type = c("point", "ellipse"), ellipse_level = 0.95, add_sample_label = "sample") +
  geom_arrow_chain(colour = "black") + 
    ggtitle("16s NMDS")
  plot
