...
```{r}
t1 <- trans_env$new(dataset = mt_mag, env_cols = c(1:19), standardize = F, complete_na = T) 

# Remove maestro temp which starts with NA
t1$data_env <- t1$data_env |> 
  dplyr::select(-matches("mtemp"))
...
# Calculate RDA plot
t1$cal_ordination(method = "dbRDA", use_measure = "robust.aitchison")
t1$trans_ordination()
...
t1$plot_ordination(plot_color = "strat_season", add_sample_label = "sample")
