...
# Create environmental object - MAGs
...
# Look at which values are shared between the two
...
# Calculate RDA plot
  ...
  # first remove the variables we don't want
t2 <- clone(t1)
...
# Create environmental object - 16S
...
#Reorder months to be in order from 8 - 7
...
#set custom breaks by binning data into 10 bins with equal number
# of observations
temp_breaks <- cut(temp_df_long$temp, 20) |> 
  ...
  str_remove("\\]") |> 
  str_squish() |> 
  # Extract everything before the comma to set breaks
  str_extract(".*(?=,)") |> 
    as.numeric() |> 
    na.omit() |> 
    # reorder numerically
